        ...
        for filename in os.listdir(directory):
            ...
            with open(file_path, 'r') as file:
                ...
                combined_content = "".join(
                    [blocks.get(line, f"{{missing block: {line}}}") for line in recipe_content])
    ...
    try:
        with open(list_file, 'r') as file:
            ...
            for line in file:
                ...
                if len(parts) != 3:
                    raise ValueError(f"Invalid line in list file: {
                                     line.strip()}. Expected format: 'blocks_dir,recipes_dir,vars_file'.")
    ...
    for key, value in vars_data.get("vars", {}).items():
        recipe_content = recipe_content.replace(
            f'{delimiters["left_delimiter"]}{key}{
                delimiters["right_delimiter"]}', value
        )
...
def main():
    ...
    parser.add_argument('--list', '-l', type=str, help="Set the list file containing directories and vars info", required=True)
    parser.add_argument('--output', '-o', type=str, help="Set the output file for rendered recipes", required=True)
